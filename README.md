# 한밭대학교 컴퓨터공학과 LLG팀
PDF 파싱 기반 RAG 파이프라인 개발

-----

**팀 구성**
- 20222039 이현정 (팀장, MLOps)
- 20201722 고동혁 (Document Parsing, Experiment)
- 20201779 이병호 (Information Retrieval)

## <u>Teamate</u> Project Background
- ### 필요성
  - 다양한 분야에서 문서 형식으로 PDF를 사용
  - 방대한 문서량으로 직접 원하는 정보를 찾아서 사용하는 데 시간이 오래 걸림
    
- ### 기존 해결책의 문제점
  - LLM은 내부 문서를 검색할 수 없으며 보안 문제가 발생할 수 있음

## System Design
   - ### System Requirements
      - Frontend: Next.js
      - Backend: FastAPI
      - Database: ChromaDB
      - Deployment: Docker, Kubernetes
        
   - ### System Features
     - PDF 문서 처리 및 텍스트 추출 기능
     - RAG(Retrieval-Augmented Generation) 기반 질의응답 시스템
     - 웹 기반 채팅 인터페이스 및 문서 관리 UI
     - ChromaDB를 활용한 벡터 데이터베이스 및 시맨틱 캐싱
     - Docker 및 Kubernetes를 통한 마이크로서비스 아키텍처 구축

   - ### System Architecture
<img width="630" height="297" alt="Image" src="https://github.com/user-attachments/assets/a4e2e7ae-bad9-486b-9c07-ebc4ce76c9d4" />


   - ### System Example
     - 기존 AI를 활용해서 질문을 하였을 때는 응답을 잘 생성하지 못하였지만, 본 RAG을 사용하였을 때는 문서 기반으로 정확하게 답변한 것을 확인할 수 있음
<img width="393" height="169" alt="Image" src="https://github.com/user-attachments/assets/0ae11774-d1d6-4e66-ad49-0e6e540ac14d" />
<img width="483" height="168" alt="Image" src="https://github.com/user-attachments/assets/ed89adb6-f2cb-4f77-8aec-f79519d47247" />
<img width="482" height="155" alt="Image" src="https://github.com/user-attachments/assets/796fb72d-f145-4ca6-80f9-4fffcbd9016c" />

  - 최종 화면
    - pdf 문서 관리 앱
    <img width="1190" height="797" alt="Image" src="https://github.com/user-attachments/assets/6ce7bb45-7f99-45ce-b7bc-2ff3cf11436b" />
    - 챗봇 UI
      
## Case Study
   - ### Description

## Conclusion
   - ### 기술적 성과
     - RAG 아키텍처를 활용한 정확도 높은 정보 검색 시스템 구현
       
   - ### 사용자 경험 개선
     - 직관적인 UI/UX를 통해 입시 정보 접근성 향상

## Project Outcome

